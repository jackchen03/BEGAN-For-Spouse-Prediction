...
parser.add_argument('--data_path', default='../dataset/img_align_celeba_64_crop/')
...
class BEGAN():
    def __init__(self):
        ...
        self.prepare_paths()
        self.data_loader = get_loader(self.data_path, opt.b_size, opt.scale_size, False, opt.num_workers)
    ...
    def load_models(self, step):
        ...
        print(os.path.join(self.gen_save_path, 'gen_%d.pth'%step) + ' loaded.')    
        self.disc.load_state_dict(torch.load(os.path.join(self.disc_save_path, 'disc_%d.pth'%step)))
        print(os.path.join(self.disc_save_path, 'disc_%d.pth'%step) + ' loaded.')
    ...
    def train(self):
        ...
        for i in range(opt.epochs):
            ...
            for _, data in enumerate(self.data_loader):
                ...
                balance = (opt.gamma*real_loss_d - fake_loss_d).item()
                opt.k += opt.lambda_k * balance
                #k = min(max(0, k), 1)
                opt.k = max(min(1, opt.k), 0)
            
                convg_measure = real_loss_d.item() + np.abs(balance) 
                ...
                if self.global_step%opt.print_step == 0:
                    print ( "Step: %d, Epochs: %d, Loss D: %.9f, real_loss: %.9f, fake_loss: %.9f, Loss G: %.9f, k: %f, M: %.9f, lr:%.9f"% (self.global_step, i, 
                                                        lossD.item(), real_loss_d.item(), fake_loss_d.item(), lossG.item(), opt.k, convg_measure, lr) )
                    self.generate(self.img_clip(gen_z), outputs_d_x, self.global_step)
...
def generative_experiments(obj):
    ...
    if opt.cuda:
        z_var = z_var.cuda()
    gen_z = obj.img_clip(obj.gen(z_var))
